...
class Navbar extends Component {
    ...
    handCheck = (e) =>{
        if ($('.navbar-mobile').hasClass('show') && !this.node_body.contains(e.target) || this.node.contains(e.target)) {
    ...
    onClickControlNavRight = (kw) =>{
        if(window.innerWidth > 728){
            if(window.innerWidth > 1024){
                if(kw === undefined){
                    if($('.main-content').hasClass('hide-menu') && !$('.main-navbar-right').hasClass('hide')){
        ...
        return (
            <div >
                <div className='navbar'>
                    ...
                    <p>
                        <i class='las la-bars' onClick={() => this.onClickControlNavRight()} ref={node => this.node = node}></i>
                    </p>
                </div>
                <div className='navbar-mobile' ref={node_body => this.node_body = node_body}>
